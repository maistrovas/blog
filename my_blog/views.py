...
def post_publish(request, pk):
	post = get_object_or_404(Post, pk=pk)
	post.publish()
	return redirect('my_blog.views.post_detail', pk=pk)

@login_required
def post_remove(request, pk):
	post = get_object_or_404(Post, pk=pk)
	post.delete()
	return redirect('my_blog.views.post_list')
...
def comment_approve(request, pk):
    ...
    comment.approve()
    return redirect('my_blog.views.post_detail', pk=comment.post.pk)
...
def comment_remove(request, pk):
    ...
    comment.delete()
    return redirect('my_blog.views.post_detail', pk=post_pk)
